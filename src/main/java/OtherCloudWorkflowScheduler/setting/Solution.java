...
public class Solution extends HashMap<VM, LinkedList<Allocation>> {
    ...
    private List<Task> sortedTask = new ArrayList<>();
    ...
    public void updateVM(VM vm) {
        ...
        for (Allocation alloc : list) {
            double newFinishTime = alloc.getTask().getInstructionSize() / vm.getMIPS() + alloc.getStartTime();
...
//                arrivalTime += inEdge.getDataSize() / VM.NETWORK_SPEED;
    ...
    public double calcVMCost(VM vm) {
        double a = vm.getUnitCost();
        double b = this.getVMLeaseEndTime(vm);
        double c = this.getVMLeaseStartTime(vm);
        double h = vm.getUnitCost() * Math.ceil((this.getVMLeaseEndTime(vm) - this.getVMLeaseStartTime(vm)) / VM.INTERVAL);
    ...
    public boolean isBetterThan(Solution s, double epsilonDeadline) {
        ...
        if (makespan1 <= epsilonDeadline && makespan2 <= epsilonDeadline) {    //both satisfy deadline
            ...
            return true;
        } else return !(makespan1 > epsilonDeadline) || !(makespan2 <= epsilonDeadline);
