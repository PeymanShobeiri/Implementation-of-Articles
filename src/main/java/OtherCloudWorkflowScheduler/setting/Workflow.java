...
//            if (source !=null) {
//                if (Objects.equals(source.getName(), "ID00029")) {
...
//            }
    ...
    private void calcTaskLevels() {
        double speed = VM.SPEEDS[VM.FASTEST] * Constants.STANDARD_MIPS;
        ...
        for (int j = this.size() - 1; j >= 0; j--) {
            ...
            for (Edge outEdge : task.getOutEdges()) {
                ...
                if (theta != Double.MAX_VALUE) {
                    double et = child.getInstructionSize() / speed;
    ...
    private class TransferData {        //this class is only used in parsing DAX data
        ...
        public void addDestination(Task t) {
            destinations.add(t);
        }

        public List<Task> getDestinations() {
            return destinations;
        }

        //-------------------------------------overrides--------------------------------
        public String toString() {
            return "TransferData [name=" + name + ", size=" + size + "]";
        }
    }
}