...
import OtherCloudWorkflowScheduler.Evaluate;
import OtherCloudWorkflowScheduler.setting.*;
...
public class ICPCP implements Scheduler {
    ...
    private final double bestVMSpeed = VM.SPEEDS[VM.FASTEST] * 100.0f;
    private Workflow wf;
    ...
    private void init() {                    //Algorithm 1 in the paper; for cases of initialization and update
        ...
        for (int i = 1; i < wf.size(); i++) {        // compute EST, EFT, critical parent via Eqs. 1 and 2; skip entry task
            ...
            if (!task.isAssigned()) {
                task.setEST(EST);
                task.setEFT(EST + task.getInstructionSize() / bestVMSpeed);
            }
        ...
        for (int j = wf.size() - 2; j >= 0; j--) {    // compute LFT via Eq. 3; reverse order, skip exit node
            ...
            for (Edge e : task.getOutEdges()) {
                ...
                if (child.isAssigned())
                    ...
                    finishTime = child.getLFT() - child.getInstructionSize() / bestVMSpeed - e.getDataSize() / VM.NETWORK_SPEED;
    ...
    private void assignPath(List<Task> PCP) {    //Algorithm 3 in the paper; the actual situation is more complex
        ...
        aa:
        for (VM vm : solution.keySet()) {            //search from existing VMs. ��Ҫ������1.ʹ��ʱ�䲻��ͻ��2.����LFT
            ...
            for (int i = 0; i < PCP.size(); i++) {
                ...
                if (taskEST + task.getInstructionSize() / vm.getMIPS() > task.getLFT() + Evaluate.E)//lft is not met, skip vm
                    continue aa;
        ...
        for (int i = 0; i < VM.TYPE_NO; i++) {        // try new VMs
            ...
            for (int k = 0; k < PCP.size(); k++) {
                ...
                if (taskEST + (task.getInstructionSize() / (VM.SPEEDS[i] * Constants.STANDARD_MIPS)) > task.getLFT() + Evaluate.E) {    //lft is not met
                    isSatisfied = false;
                    break;
                }
        ...
        for (Allocation alloc : bestList) {
            alloc.setFinishTime(alloc.getStartTime() + alloc.getTask().getInstructionSize() / alloc.getVM().getMIPS());
        ...
        Collections.sort(allocList, (o1, o2) -> {
    ...
    private double searchStartTime(VM vm, Task task, double EST, double LFT) {
        ...
        for (int i = 0; i < list.size() + 1; i++) {
            ...
            double slackTime = LFT - EST - task.getInstructionSize() / vm.getMIPS();
            if (EST + slackTime >= timeSlotStart) {            //condition1��startTime satisfies
                ...
                if (timeSlotEnd - startTime >= task.getInstructionSize() / vm.getMIPS())
                    return startTime;
            }
        }
        return -1;
