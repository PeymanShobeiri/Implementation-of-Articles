...
public class PcpPolicy2 extends WorkflowPolicy {
    boolean backCheck = true;

    public PcpPolicy2(WorkflowGraph g, ResourceSet rs, long bw) {
        super(g, rs, bw);
        ...
        for (Link child : parentNode.getChildren()) {
            ...
            if (childNode.getEST() < newEST) {
                ...
                if (childNode.getLFT() < childNode.getEFT())
                    System.out.println("EST Setting: ID=" + childNode.getId() + " EFT=" + childNode.getEFT() + " LFT=" + childNode.getLFT());
